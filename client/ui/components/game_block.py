...
class GameBlock(customtkinter.CTkFrame):
    def __init__(self, master, game_name: str, version: str, min_players: int, max_players: int, game_cover: Optional[bytes] = None, command: Optional[Callable[[], None]] = None):
        ...
        if game_cover:
            image_data = BytesIO(game_cover)
            # pil_image = Image.open(image_data).resize((256, 256))
            # self.game_cover_image = customtkinter.CTkImage(pil_image, size=(256, 256))
        else:
            with open("client/ui/assets/default_game_cover.png", "rb") as f:
                default_image_data = f.read()
            image_data = BytesIO(default_image_data)

        pil_image = Image.open(image_data).resize((128, 128))
        self.game_cover_image = customtkinter.CTkImage(pil_image, size=(128, 128))
